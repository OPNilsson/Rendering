...
public:
    ...
    bool trace_refracted(const optix::Ray &in, const HitInfo &in_hit, optix::Ray &out, HitInfo &out_hit,
                         float &fresnel_R) const;

private:
    float get_ior_out(const optix::Ray &in, const HitInfo &in_hit, optix::float3 &normal) const;
};
