...
void Scene::add_plane(const float3 &position, const float3 &normal, const string &mtl_file, unsigned int idx,
                      float tex_scale) {
...
void
Scene::add_triangle(const float3 &v0, const float3 &v1, const float3 &v2, const string &mtl_file, unsigned int idx) {
    vector<ObjMaterial> m;
    if (!mtl_file.empty())
        mtl_load(mtl_file, m);
    if (m.size() == 0)
