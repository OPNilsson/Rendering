...
public:
    ...
    optix::Matrix3x3 get_matrix3x3() const {
        ...
        result.setRow(0, optix::make_float3(1.0f - s * (qv_qv.y + qv_qv.z), s * (qv.x * qv.y - qv_qw.z),
                                            s * (qv.x * qv.z + qv_qw.y)));
        result.setRow(1, optix::make_float3(s * (qv.x * qv.y + qv_qw.z), 1.0f - s * (qv_qv.x + qv_qv.z),
                                            s * (qv.y * qv.z - qv_qw.x)));
        result.setRow(2, optix::make_float3(s * (qv.x * qv.z - qv_qw.y), s * (qv.y * qv.z + qv_qw.x),
                                            1.0f - s * (qv_qv.x + qv_qv.y)));
        return result;
    }
    ...
    optix::Matrix4x4 get_matrix4x4() const {
        ...
        // note that the all q_*q_ are used twice (optimize)
        result.setRow(0, optix::make_float4(1.0f - s * (qv_qv.y + qv_qv.z), s * (qv.x * qv.y - qv_qw.z),
                                            s * (qv.x * qv.z + qv_qw.y), 0.0f));
        result.setRow(1, optix::make_float4(s * (qv.x * qv.y + qv_qw.z), 1.0f - s * (qv_qv.x + qv_qv.z),
                                            s * (qv.y * qv.z - qv_qw.x), 0.0f));
        result.setRow(2, optix::make_float4(s * (qv.x * qv.z - qv_qw.y), s * (qv.y * qv.z + qv_qw.x),
                                            1.0f - s * (qv_qv.x + qv_qv.y), 0.0f));
    ...
    void make_rot(const optix::Matrix3x3 &m) {
        ...
        if (trace > 1.0e-7f) {
            ...
            qv = optix::make_float3(m.getRow(2).y - m.getRow(1).z, m.getRow(0).z - m.getRow(2).x,
                                    m.getRow(1).x - m.getRow(0).y);
            qv /= S;
            qw = 0.25f * S;
        } else {
