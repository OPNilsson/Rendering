...
class Scene {
public:
    Scene(Camera *c) : cam(c), shaders(10, static_cast<Shader *>(0)), redraw(true), do_textures(false) {}
    ...
    void add_plane(const optix::float3 &position, const optix::float3 &normal, const std::string &mtl_file,
                   unsigned int idx = 0, float tex_scale = 1.0f);

    void add_sphere(const optix::float3 &center, float radius, const std::string &mtl_file, unsigned int idx = 0);

    void
    add_triangle(const optix::float3 &v0, const optix::float3 &v1, const optix::float3 &v2, const std::string &mtl_file,
                 unsigned int idx = 0);
