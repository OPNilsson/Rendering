...
bool Plane::intersect(const Ray& r, HitInfo& hit, unsigned int prim_idx) const
{
  ...
  // If the ray is parallel to the plane, there is no intersection
    ...
    float distance = -((dot(r.origin, onb.m_normal) + d) / dot(r.direction, onb.m_normal));

    // Check if the ray is in the plane
    if (dot(position - r.origin, onb.m_normal) == 0.0f){
        // If the ray is in the plane, the intersection point is the ray origin
        hit.has_hit = true;
        hit.dist = 0.0f;
        ...
        if (material.has_texture)
        {
            ...
            get_uv(r.origin, hit.texcoord.x, hit.texcoord.y);
        }

        return true;
    }
    // Check if the t value is within the ray's tmin and tmax
    else if (distance >= r.tmin && distance <= r.tmax)
    {
        // Calculate the intersection point
        float3 intersection_point = r.origin + r.direction * d;
        ...
        {
            // Calculate the texture coordinates
            get_uv(intersection_point, hit.texcoord.x, hit.texcoord.y);
        }
