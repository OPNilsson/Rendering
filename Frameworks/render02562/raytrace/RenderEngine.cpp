...
RenderEngine::RenderEngine()
        : win(optix::make_uint2(512, 512)),                        // Default window size
          ...
          holdout(&tracer, scene.get_lights(),
                  1),                 // No. of samples per path in holdout ambient occlusion
          mirror(&tracer),
          transparent(&tracer),
          volume(&tracer),
    ...
    if (argc > 1) {
        for (int i = 1; i < argc; ++i) {
            ...
            if (char_traits<char>::compare(filename.c_str(), "cornell", 7) == 0)
                transform =
                        Matrix4x4::scale(make_float3(0.025f)) * Matrix4x4::rotate(M_PIf, make_float3(0.0f, 1.0f, 0.0f));
            else if (char_traits<char>::compare(filename.c_str(), "bunny", 5) == 0)
                transform =
                        Matrix4x4::translate(make_float3(-3.0f, -0.85f, -8.0f)) * Matrix4x4::scale(make_float3(25.0f));
            else if (char_traits<char>::compare(filename.c_str(), "justelephant", 12) == 0)
                transform = Matrix4x4::translate(make_float3(-10.0f, 3.0f, -2.0f)) *
                            Matrix4x4::rotate(0.5f, make_float3(0.0f, 1.0f, 0.0f));
        ...
        scene.add_plane(make_float3(0.0f, 0.0f, 0.0f), make_float3(0.0f, 1.0f, 0.0f), "../models/default_scene.mtl", 1,
                        0.2f * texture_scale); // last argument is texture scale
        ...
        scene.add_triangle(make_float3(-0.2f, 0.1f, 0.9f), make_float3(0.2f, 0.1f, 0.9f),
                           make_float3(-0.2f, 0.1f, -0.1f), "../models/default_scene.mtl", 3);
...
void RenderEngine::init_tracer() {
    ...
    unsigned int lights_in_scene = scene.extract_area_lights(&tracer,
                                                             1);  // Set number of samples per light source here
...
#pragma omp parallel for private(randomizer)
    for (int y = 0; y < static_cast<int>(res.y); ++y) {
        ...
        for (int x = 0; x < static_cast<int> (res.x); x++) {
            ...
            float2 coords = 1.0f / res.y * make_float2(x, y) +
                            0.5f * make_float2(1.0f / res.x - aspect_ratio, 1.0f / res.y - 1.0f);
