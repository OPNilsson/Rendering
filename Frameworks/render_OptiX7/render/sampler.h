...
__inline__ __device__ float3 sample_disk(const float3& center, const float3& normal, float radius, unsigned int& t)
{
  // Get random numbers
  float sin_theta = sqrtf(rnd(t));
  float phi = 2.0f*M_PIf*rnd(t);

  // Calculate vector to point in the disk as if the z-axis were the normal
  float3 v = spherical_direction(radius*sin_theta, 0.0f, phi);
