...
__host__ __device__ __inline__ float3 fresnel_complex_R(float cos_theta, const float3& eta_sq, const float3& kappa_sq)
{
  ...
  float3 z_real = eta_sq - kappa_sq - sin_theta_sqr;
  float3 z_imag_sqr = 4.0f*eta_sq*kappa_sq;
  float3 abs_z = sqrtf(z_real*z_real + z_imag_sqr);
  float3 two_a = sqrtf(2.0f*(abs_z + z_real));

  float3 c1 = abs_z + cos_theta_sqr;
  float3 c2 = two_a*cos_theta;
  float3 R_s = (c1 - c2)/(c1 + c2);

  c1 = abs_z + sin_theta_sqr*tan_theta_sqr;
  c2 = two_a*sin_theta_sqr/cos_theta;
  float3 R_p = R_s*(c1 - c2)/(c1 + c2);
...
{
  return -1.440*n1_over_n2*n1_over_n2 + 0.710*n1_over_n2 + 0.668 + 0.0636/n1_over_n2;
}
...
{
  // Compute the Fresnel reflectance using fresnel_r_s(...) and fresnel_r_p(...)
  m_complex r_s = fresnel_r_s(cos_theta1, cos_theta2, ior1_over_ior2);
  m_complex r_p = fresnel_r_p(cos_theta1, cos_theta2, ior1_over_ior2);
...
{
  m_complex cos_theta_t = sqrt(1.0f - ior1_over_ior2*ior1_over_ior2*(1.0f - cos_theta_i*cos_theta_i));
...
{
  m_complex ior2_over_ior1 = 1.0f/ior1_over_ior2;
  float cos_theta_i_sqr = cos_theta_i*cos_theta_i;
  m_complex cos_theta_t_sqr = 1.0f - ior1_over_ior2*ior1_over_ior2*(1.0f - cos_theta_i_sqr);
  m_complex cos_theta_t = sqrt(cos_theta_t_sqr);
...
__host__ __device__ __inline__ m_complex fresnel_T_complex(float cos_theta_i, m_complex ior1_over_ior2)
{
  ...
  m_complex cos_theta_t_sqr = 1.0f - ior1_over_ior2*ior1_over_ior2*(1.0f - cos_theta_i_sqr);
  m_complex cos_theta_t = sqrt(cos_theta_t_sqr);
  m_complex i_m_t = cos_theta_i*cos_theta_t;
  m_complex i2_p_t2 = cos_theta_i_sqr + cos_theta_t_sqr;
  m_complex numerator = (cos_theta_i_sqr + i_m_t)*(1.0f + ior2_over_ior1);
