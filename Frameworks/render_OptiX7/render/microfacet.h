                                        ...
                                        float cos_theta_i, float cos_theta_o, const float ior1_over_ior2)
{
  ...
  float F = 1.0f;
  if(transmit)
    F = fresnel_R(fabsf(wi_dot_wh), fabsf(cos_theta_o), ior1_over_ior2);
  else
  {
                                ...
                                float cos_theta_o, const float ior1_over_ior2, const float xi)
{
  ...
  float cos_theta_t = sqrtf(1.0 - sin_theta_t_sqr);
  float R = sin_theta_t_sqr >= 1.0 ? 1.0f : fresnel_R(fabsf(wo_dot_wh), cos_theta_t, ior1_over_ior2);
...
__device__ float blinn_refract(float3& wh, float3& wi, const float3& wo, const float3& n,
                               float cos_theta_o, const float ior1_over_ior2, const float s, unsigned int& t)
{
  wh = sample_Blinn_normal(n, s, t);
...
__device__ float blinn_smith_G_refract(float3& wh, float3& wi, const float3& wo, const float3& n,
                                       float cos_theta_o, const float ior1_over_ior2, const float s, unsigned int& t)
{
  wh = sample_Blinn_normal(n, s, t);
  float2 cosines = rough_refract(wi, wo, wh, n, cos_theta_o, ior1_over_ior2, rnd(t));
...
__device__ float beckmann_refract(float3& wh, float3& wi, const float3& wo, const float3& n,
                                  float cos_theta_o, const float ior1_over_ior2, const float s, unsigned int& t)
{
  wh = sample_Beckmann_normal(n, s, t);
  float2 cosines = rough_refract(wi, wo, wh, n, cos_theta_o, ior1_over_ior2, rnd(t));
  const float cos_theta_i = cosines.x;
  const float cosine_weight = cosines.y;
...
__device__ float ggx_refract(float3& wh, float3& wi, const float3& wo, const float3& n,
                             float cos_theta_o, const float ior1_over_ior2, const float s, unsigned int& t)
{
  wh = sample_GGX_normal(n, s, t);
  float2 cosines = rough_refract(wi, wo, wh, n, cos_theta_o, ior1_over_ior2, rnd(t));
  const float cos_theta_i = cosines.x;
  const float cosine_weight = cosines.y;
...
__device__ float3 blinn_refract(float3& wh, float3& wi, const float3& wo, const float3& n,
...
__device__ float3 blinn_smith_G_refract(float3& wh, float3& wi, const float3& wo, const float3& n,
                                       float cos_theta_o, const complex3& recip_ior, const float s, unsigned int& t)
{
  wh = sample_Blinn_normal(n, s, t);
  float3 F;
...
__device__ float3 beckmann_refract(float3& wh, float3& wi, const float3& wo, const float3& n,
                                  float cos_theta_o, const complex3& recip_ior, const float s, unsigned int& t)
{
  wh = sample_Beckmann_normal(n, s, t);
  float3 F;
...
__device__ float3 ggx_refract(float3& wh, float3& wi, const float3& wo, const float3& n,
                             float cos_theta_o, const complex3& recip_ior, const float s, unsigned int& t)
{
  wh = sample_GGX_normal(n, s, t);
  float3 F;
